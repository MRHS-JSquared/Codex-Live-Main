...
export const TeamProvider = ({ children }: { children: ReactNode }) => {
  ...
  const leaveTeam = () => {
    ...
    // Clear the team from memory
    setTeam(null);

    // Remove teamCode from user and update localStorage
    const updatedUser = { ...user, teamCode: undefined };
    localStorage.setItem("codex-user", JSON.stringify(updatedUser));
    updateUserInList(updatedUser);
  };

  return (
    <TeamContext.Provider value={{ team, createTeam, joinTeam, leaveTeam }}>
      {children}
